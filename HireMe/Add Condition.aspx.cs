    ...
    public partial class Add_Condition : System.Web.UI.Page
    {
        ...
        protected void btn_addCond_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 0; j < data_emp_cond.Rows.Count; j++)
            {
                ...
                if (id_vac == select_spec.SelectedValue)
                {
                    if (cond_type == select_type.Value)
                    {
                        //الشرط موجود
            ...
            try
            {
                data_Access.EX_Non_Query_Insert("INSERT INTO tb_emp_condition(id_vacancy,emp_condition_name,emp_condition_type) VALUES('" + select_spec.SelectedValue + "','" + condiname.Text + "','" + select_type.Value + "')");
